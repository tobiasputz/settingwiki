# Seeker 7.3.2 — Source-scoped Homebrew


Compendium separates campaign lore from custom rules. A GM can use the three-dot menu beside any `.tex` file and classify the **entire source** as an ancestry, archetype, class, item/rules source, or other Homebrew without moving or rewriting it; the original LaTeX remains exactly where it already participates in PDF compilation. Homebrew then exposes one source-backed entry (for example **Jotunari**) with its lore/heritage pages and a normalized PF2e rules list collected from native `\feat` / `\action` commands. Ancestries and archetypes have separate library sections and can be imported to Foundry as managed rule bundles.

Homebrew Forge entries can also be written into an existing `.tex` file at a chosen part/chapter/section/subsection. Stable Seeker markers make re-export an update rather than an append, and matching manually-authored commands are detected so source-backed and Forge representations do not appear twice in the Homebrew library. The Monster Codex remains a clean Seeker-native dossier with GM-controlled disclosure, and Studio still safely moves/renames project files while rewriting ordinary project-local `\input`, `\include`, and `\subfile` references.
