...
def init_v5_db(settings: Settings) -> None:
    with connect(settings) as conn:
        ...
        prep_cols={r[1] for r in conn.execute("PRAGMA table_info(session_preparations)").fetchall()}
        if prep_cols and "pacing_json" not in prep_cols:
...
def get_preparation(settings: Settings, session_id: int) -> dict:
    ...
    if not row:return {'session_id':int(session_id),'opening':'','beats':[],'secrets':'','contingencies':'','notes':'','references':[],'pacing':[]}
    out=dict(row);out['beats']=_json(out.pop('beats_json','[]'),[]);out['references']=_json(out.pop('references_json','[]'),[]);out['pacing']=_json(out.pop('pacing_json','[]'),[]);return out
...
def save_preparation(settings: Settings, session_id: int, campaign_id: int, payload: dict) -> dict:
    ...
    with connect(settings) as conn:
        ...
        pacing=payload.get('pacing') if isinstance(payload.get('pacing'),list) else []
        clean_pacing=[]
        for step in pacing[:20]:
            if isinstance(step,dict):
                clean_pacing.append({'label':str(step.get('label') or '')[:160],'done':bool(step.get('done')),'note':str(step.get('note') or '')[:1000]})
            elif str(step).strip():
                clean_pacing.append({'label':str(step)[:160],'done':False,'note':''})
        conn.execute("INSERT OR REPLACE INTO session_preparations(session_id,campaign_id,opening,beats_json,secrets,contingencies,notes,references_json,pacing_json,updated_at) VALUES(?,?,?,?,?,?,?,?,?,?)",
                     (int(session_id),int(campaign_id),str(payload.get('opening') or '')[:12000],json.dumps(clean_beats),str(payload.get('secrets') or '')[:16000],str(payload.get('contingencies') or '')[:12000],str(payload.get('notes') or '')[:24000],json.dumps(clean_refs),json.dumps(clean_pacing),now))
