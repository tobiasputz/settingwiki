...
def register_v7_routes(app, settings: Settings, templates, helpers: dict[str, Callable[..., Any]]) -> None:
    ...
    def v7_health(request:Request):
        ...
        return {'ok':True,'version':'10.2.0','campaign_id':cid,'entities':tracked,'objects':tracked,'foundry_queue':queue,'asset_bytes':assets['total_bytes'],'asset_files':assets['count'],'dependency_warnings':len(dependency_warnings(settings,cid)),'migration_backups':len(list(backups.glob('pre-v7-*.sqlite'))) if backups.exists() else 0}
