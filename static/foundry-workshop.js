(()=>{
  ...
  const kindName=k=>({monster:'Creature',npc:'NPC',item:'Item',feat:'Feat',action:'Action',ancestry:'Ancestry',archetype:'Archetype',background:'Background',spell:'Spell',hazard:'Hazard',class:'Class',homebrew:'Freeform'}[k]||'Homebrew');
  const kindIcon=k=>({monster:'♜',npc:'♟',item:'◇',feat:'✦',action:'◆',ancestry:'♜',archetype:'⌘',background:'◈',spell:'✧',hazard:'⚠',class:'♛',homebrew:'◇'}[k]||'◇');
  const displayKind=e=>{const doc=String(e?.payload?.homebrew_document||'').toLowerCase();return e?.kind==='homebrew'&&['ancestry','archetype','background','spell','hazard','class'].includes(doc)?doc:(e?.kind||'homebrew')};
  ...
  function setKind(kind,{resetCollections=false}={}){
    ...
    updateItemSubtype();updateHomebrewDocument();updateToolbar();renderPreview();setDirty();
  }
  ...
  function updateHomebrewDocument(){
    const doc=fieldValue('kind')==='homebrew'?String(fieldValue('homebrew_document')||'equipment').toLowerCase():'';
    $$('[data-fw-doc]').forEach(el=>el.classList.toggle('fw-hidden',el.dataset.fwDoc!==doc));
    const structured=['background','spell','hazard','class'];$$('[data-fw-doc-kind]').forEach(b=>b.classList.toggle('active',b.dataset.fwDocKind===doc));const free=$('[data-fw-kind="homebrew"]');if(free&&fieldValue('kind')==='homebrew')free.classList.toggle('active',!structured.includes(doc));
    if(fieldValue('kind')==='homebrew'&&structured.includes(doc)){editingLabel.textContent=`${state.currentId?'EDITING':'NEW'} ${kindName(doc).toUpperCase()}`;toolbarTitle.textContent=fieldValue('title').trim()||`Untitled ${kindName(doc).toLowerCase()}`}
  }
  ...
  function collectData(sync=true){
    ...
    if(uiKind==='homebrew'&&payload.homebrew_document==='class'){payload.library_section='class';payload.library_group=fieldValue('title').trim();payload.class_name=fieldValue('title').trim()}
  ...
  function validateData(data,{announce=false}={}){
    ...
    if(data?.kind==='homebrew') {
      const doc=String(p.homebrew_document||'').toLowerCase();
      if(doc==='background'){if(!String(p.background_boosts||'').trim())problems.push('Background needs ability boosts.');if(!String(p.background_skill||'').trim())problems.push('Background needs a trained skill.');}
      if(doc==='spell'){numberProblem('spell_rank','Spell rank',0,10);if(!String(p.spell_traditions||'').trim()&&String(p.spell_category||'')!=='focus')problems.push('Spell needs at least one tradition (unless it is intentionally focus-only).');}
      if(doc==='hazard'){numberProblem('hazard_ac','Hazard AC',0,80);numberProblem('hazard_hp','Hazard HP',0,99999);numberProblem('hazard_hardness','Hazard hardness',0,9999);if(!String(p.hazard_disable||'').trim())problems.push('Hazard needs a Disable method.');}
      if(doc==='class'){numberProblem('class_hp','Class HP',1,30);if(!String(p.class_key_ability||'').trim())problems.push('Class needs a key ability.');const seen=new Set();(p.bundle_feats||[]).forEach((f,i)=>{const row=$(`[data-bundle-feat-index="${i}"]`),title=String(f.title||'').trim();if(!title){problems.push(`Class feat ${i+1} needs a name.`);addRowProblem(row,'Add a feat name.')}else if(seen.has(title.toLowerCase())){problems.push(`Duplicate class feat: ${title}.`);addRowProblem(row,'Duplicate feat name.')}else seen.add(title.toLowerCase())});}
    }
  ...
  function fillForm(entry=null){
    ...
    if(!entry){setField('level','1');setField('rarity','common');setField('size','med');setField('quantity','1');setField('ancestry_hp','8');setField('ancestry_size','med');setField('ancestry_speed','25');setField('ancestry_reach','5');setField('ancestry_vision','normal');setField('ancestry_languages','common');setField('ancestry_additional_languages','0');setField('ancestry_free_boosts','2');setField('dedication_level','2');setField('dedication_traits','archetype, dedication');setField('spell_rank','1');setField('spell_category','spell');setField('spell_actions','2');setField('hazard_complexity','simple');setField('class_hp','8');setField('weapon_category','simple');setField('weapon_damage_dice','1');setField('weapon_damage_die','d6');setField('weapon_damage_type','slashing');setField('weapon_damage_modifier','0');setField('weapon_bonus','0');setField('weapon_usage','held-in-one-hand');setField('weapon_potency','0');setField('weapon_striking','0');['str_mod','dex_mod','con_mod','int_mod','wis_mod','cha_mod'].forEach(k=>setField(k,'0'))}
    ...
    setKind(inferred);renderRepeaters();updateItemSubtype();updateHomebrewDocument();updateArtStatus();updateToolbar();updateSourceLinkUI();renderPreview();
  ...
  function libraryMeta(e){const p=e.payload||{},k=displayKind(e);if(k==='ancestry')return ['Ancestry',`${(p.heritages||[]).length} heritages`,`${(p.bundle_feats||[]).length} feats`];if(k==='archetype')return ['Archetype',`Dedication ${p.dedication_level||2}`,`${(p.bundle_feats||[]).length} feats`];if(k==='class')return ['Class',`${(p.bundle_feats||[]).length} feats`,p.class_key_ability||''];if(k==='spell')return ['Spell',`Rank ${p.spell_rank||p.level||1}`,p.spell_category||''];if(k==='hazard')return ['Hazard',`Level ${p.level||0}`,p.hazard_complexity||'simple'];if(k==='background')return ['Background',p.background_skill||'',p.background_skill_feat||''];const lv=String(p.level??'').trim();return [kindName(k),lv!==''?`Level ${lv}`:'',p.rarity&&p.rarity!=='common'?p.rarity:''].filter(Boolean)}
  ...
  function structuredDocumentPreview(d,doc){const p=d.payload||{},title=esc(d.title||`Untitled ${doc}`),desc=p.description?`<p class="fw-stat-desc">${lines(p.description)}</p>`:'';if(doc==='background')return `<header class="fw-stat-top fw-free-top"><h3>${title}</h3><span class="fw-stat-level">BACKGROUND</span></header><div class="fw-stat-body"><p><strong>Ability Boosts</strong> ${esc(p.background_boosts||'—')}</p><p><strong>Trained Skill</strong> ${esc(p.background_skill||'—')}</p>${p.background_lore?`<p><strong>Lore</strong> ${esc(p.background_lore)}</p>`:''}${p.background_skill_feat?`<p><strong>Skill Feat</strong> ${esc(p.background_skill_feat)}</p>`:''}<hr class="fw-stat-divider">${desc}${p.background_special?`<p>${lines(p.background_special)}</p>`:''}</div>`;if(doc==='spell')return `<header class="fw-stat-top fw-feat-top"><h3>${title} ${actionGlyph(p.spell_actions)?`<span class="fw-action-glyphs">${actionGlyph(p.spell_actions)}</span>`:''}</h3><span class="fw-stat-level">SPELL ${esc(p.spell_rank||1)}</span></header><div class="fw-preview-traits fw-feat-traits">${slugList(p.traits).map(t=>`<span>${esc(t)}</span>`).join('')}</div><div class="fw-stat-body"><p><strong>Traditions</strong> ${esc(p.spell_traditions||'—')}</p>${p.spell_range?`<p><strong>Range</strong> ${esc(p.spell_range)}</p>`:''}${p.spell_targets?`<p><strong>Targets</strong> ${esc(p.spell_targets)}</p>`:''}${p.spell_area?`<p><strong>Area</strong> ${esc(p.spell_area)}</p>`:''}${p.spell_defense?`<p><strong>Defense</strong> ${esc(p.spell_defense)}</p>`:''}${p.spell_duration?`<p><strong>Duration</strong> ${esc(p.spell_duration)}</p>`:''}<hr class="fw-stat-divider">${desc}${p.spell_heightened?`<p><strong>Heightened</strong> ${lines(p.spell_heightened)}</p>`:''}</div>`;if(doc==='hazard')return `<header class="fw-stat-top fw-free-top"><h3>${title}</h3><span class="fw-stat-level">HAZARD ${esc(p.level||0)}</span></header><div class="fw-stat-body"><p><strong>${esc(p.hazard_complexity||'simple')} hazard</strong>${p.hazard_stealth?` · <strong>Stealth</strong> ${esc(p.hazard_stealth)}`:''}</p>${p.hazard_disable?`<p><strong>Disable</strong> ${lines(p.hazard_disable)}</p>`:''}<div class="fw-bundle-preview-stats"><span><strong>AC</strong>${esc(p.hazard_ac||'—')}</span><span><strong>HP</strong>${esc(p.hazard_hp||'—')}</span><span><strong>Hardness</strong>${esc(p.hazard_hardness||'—')}</span></div><hr class="fw-stat-divider">${desc}${p.hazard_routine?`<p><strong>Routine</strong> ${lines(p.hazard_routine)}</p>`:''}${abilityPreview(p.abilities)}</div>`;if(doc==='class')return `<header class="fw-stat-top fw-free-top"><h3>${title}</h3><span class="fw-stat-level">CLASS</span></header><div class="fw-stat-body"><div class="fw-bundle-preview-stats"><span><strong>HP</strong>${esc(p.class_hp||8)}</span><span><strong>Key Ability</strong>${esc(p.class_key_ability||'—')}</span><span><strong>Class DC</strong>${esc(p.class_dc||'—')}</span></div>${desc}${p.class_progression?`<p>${lines(p.class_progression)}</p>`:''}<hr class="fw-stat-divider"><h4>Class feats · ${(p.bundle_feats||[]).length}</h4>${(p.bundle_feats||[]).slice().sort((a,b)=>Number(a.level||0)-Number(b.level||0)).slice(0,12).map(f=>`<div class="fw-stat-action"><h4>${esc(f.title||'Feat')}</h4><small>Level ${esc(f.level||1)}</small>${bundleRuleMetaHtml(f)}<p>${lines(f.description||'')}</p></div>`).join('')}</div>`;return homebrewPreview(d)}
  function renderPreview(){
    ...
    const k=fieldValue('kind'),doc=String(d.payload?.homebrew_document||'').toLowerCase();preview.innerHTML=k==='ancestry'?ancestryPreview(d):k==='archetype'?archetypePreview(d):creatureKind(d.kind)?creaturePreview(d):d.kind==='item'?itemPreview(d):d.kind==='feat'?featPreview(d):d.kind==='action'?homebrewPreview({...d,payload:{...d.payload,homebrew_document:'action',homebrew_actions:d.payload?.action_cost}}):(d.kind==='homebrew'&&['background','spell','hazard','class'].includes(doc)?structuredDocumentPreview(d,doc):homebrewPreview(d));
  }
  ...
  $$('[data-fw-template]').forEach(b=>b.addEventListener('click',()=>{
    ...
    const presets={
      ...
      'background-standard':{homebrew_document:'background',library_section:'other',background_boosts:'Free, Free',summary:'A PF2e background with ability boosts, trained skills, Lore, and a skill feat.'},
      'spell-standard':{homebrew_document:'spell',library_section:'other',spell_rank:'1',spell_category:'spell',spell_actions:'2',summary:'A standalone PF2e spell with structured casting, range, target, defense, duration, and heightening.'},
      'hazard-standard':{homebrew_document:'hazard',library_section:'other',hazard_complexity:'simple',summary:'A PF2e hazard with detection, disable methods, defenses, routine, and reset rules.'},
      'class-standard':{homebrew_document:'class',library_section:'class',class_hp:'8',summary:'A PF2e class chassis with proficiencies, progression, and a class feat line.'},
    ...
    if(key==='class-standard'&&!state.bundleFeats.length)state.bundleFeats=[{title:'',level:'1',action_cost:'',traits:'class',access:'',prerequisites:'',frequency:'',trigger:'',requirements:'',special:'',description:''}];
    updateItemSubtype();updateHomebrewDocument();renderRepeaters();renderPreview();setDirty();toast('Quick-start template applied. Fill in level-appropriate numbers.');
  }));
  ...
  $$('[data-fw-add-bundle-feat]').forEach(b=>b.addEventListener('click',()=>{syncRepeaters();const doc=String(fieldValue('homebrew_document')||'');const arche=fieldValue('kind')==='archetype';const classDoc=fieldValue('kind')==='homebrew'&&doc==='class';state.bundleFeats.push({title:'',level:arche?'4':'1',action_cost:'',traits:arche?'archetype':classDoc?'class':'',access:'',prerequisites:'',frequency:'',trigger:'',requirements:'',special:'',description:''});renderRepeaters();renderPreview();setDirty();const host=classDoc?$('[data-fw-doc="class"] [data-fw-bundle-feats]'):$('[data-fw-section="'+fieldValue('kind')+'"] [data-fw-bundle-feats]');$('.fw-repeat-row:last-child input',host)?.focus()}));
  ...
  async function loadExtensionForgeTypes(query){
    try{
      const data=await jsonFetch('/api/v9/extensions'), picker=$('.fw-type-picker');
      const requestedPlugin=String(query.get('ext_plugin')||''), requestedType=String(query.get('ext_type')||'');
      for(const ext of data.extensions||[]){
        for(const ft of ext.forge_types||[]){
          if(!ft?.id||!ft?.title)continue;
          const b=document.createElement('button');b.type='button';b.dataset.fwExtension=`${ext.id}:${ft.id}`;b.innerHTML=`<span>${esc(ft.icon||'◇')}</span><strong>${esc(ft.title)}</strong><small>${esc(ft.description||ext.title)}</small>`;picker?.append(b);
          const apply=()=>{setKind('homebrew',{resetCollections:true});setField('homebrew_document',String(ft.document||'equipment'));setField('extension_plugin',ext.id);setField('extension_type',ft.id);setField('library_group',ft.group||ft.title);const preset=ft.preset&&typeof ft.preset==='object'?ft.preset:{};Object.entries(preset).forEach(([k,v])=>setField(k,Array.isArray(v)?v.join(', '):v));setField('extension_fields',JSON.stringify(ft.fields||{}));updateHomebrewDocument();renderPreview();setDirty();toast(`${ft.title} Forge preset loaded.`)};
          b.addEventListener('click',apply);
          if(requestedPlugin===String(ext.id)&&requestedType===String(ft.id))setTimeout(apply,0);
        }
      }
    }catch(err){console.debug('Extension Forge types unavailable',err)}
  }
  ...
  else if(['background','spell','hazard','class'].includes(requestedNew)){fillForm();state.currentId=null;setKind('homebrew',{resetCollections:true});setField('homebrew_document',requestedNew);if(requestedNew==='class')setField('library_section','class');updateHomebrewDocument();state.savedSnapshot='';renderPreview();setDirty()}
