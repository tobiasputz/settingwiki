(()=>{
  ...
  function spellRow(row={},index=0){
    return `<div class="fw-repeat-row spell" data-spell-index="${index}">
      <label><span>Spell name</span><input data-rkey="name" value="${esc(row.name||'')}" placeholder="Fireball"></label>
      <label><span>Rank</span><input data-rkey="rank" type="number" min="0" max="10" step="1" value="${esc(row.rank??'')}" placeholder="3"></label>
      <label><span>Cast</span><select data-rkey="actions"><option value="1" ${row.actions==='1'?'selected':''}>◆ 1</option><option value="2" ${!row.actions||row.actions==='2'?'selected':''}>◆◆ 2</option><option value="3" ${row.actions==='3'?'selected':''}>◆◆◆ 3</option><option value="reaction" ${row.actions==='reaction'?'selected':''}>↺ Reaction</option><option value="free" ${row.actions==='free'?'selected':''}>◇ Free</option><option value="1 minute" ${row.actions==='1 minute'?'selected':''}>1 minute</option></select></label>
      ...
      <label><span>Target / area</span><input data-rkey="target" value="${esc(row.target||'')}" placeholder="20-foot burst"></label>
      <label><span>Defense</span><select data-rkey="save"><option value="" ${!row.save?'selected':''}>None / attack</option><option value="fortitude" ${row.save==='fortitude'?'selected':''}>Fortitude</option><option value="reflex" ${row.save==='reflex'?'selected':''}>Reflex</option><option value="will" ${row.save==='will'?'selected':''}>Will</option><option value="ac" ${row.save==='ac'?'selected':''}>AC</option></select></label>
      ...
      <label class="fw-repeat-traits"><span>Traits</span><input data-rkey="traits" value="${esc(row.traits||'')}" placeholder="fire, manipulate, concentrate"></label>
      <label><span>Duration</span><input data-rkey="duration" value="${esc(row.duration||'')}" placeholder="1 minute"></label>
      <textarea data-rkey="description" placeholder="Spell effect / rules text…">${esc(row.description||'')}</textarea>
    </div>`;
  }
  ...
  function spellPreview(p){
    ...
    const head=`${tradition.charAt(0).toUpperCase()+tradition.slice(1)} ${String(p.spell_mode||'innate').replace(/^./,c=>c.toUpperCase())} Spells${String(p.spell_dc||'').trim()?` DC ${esc(p.spell_dc)}`:''}${String(p.spell_attack||'').trim()?`, attack ${signed(p.spell_attack)}`:''}`;
  ...
  $('[data-fw-add-spell]')?.addEventListener('click',()=>{syncRepeaters();state.spells.push({name:'',rank:'1',actions:'2',range:'',target:'',save:'',damage:'',damage_type:'fire',traits:'',duration:'',description:''});renderRepeaters();renderPreview();setDirty();$('[data-fw-spells] .fw-repeat-row:last-child input')?.focus()});
  root.addEventListener('click',e=>{const ra=e.target.closest('[data-remove-attack]');if(ra){syncRepeaters();state.attacks.splice(+ra.dataset.removeAttack,1);renderRepeaters();renderPreview();setDirty()}const rb=e.target.closest('[data-remove-ability]');if(rb){syncRepeaters();state.abilities.splice(+rb.dataset.removeAbility,1);renderRepeaters();renderPreview();setDirty()}const rs=e.target.closest('[data-remove-spell]');if(rs){syncRepeaters();state.spells.splice(+rs.dataset.removeSpell,1);renderRepeaters();renderPreview();setDirty()}});
  ...
  const tokenSettings={shape:'circle',ring:'#b79661',bg:'#171513',accent:'#e3c68f',scale:1,x:0,y:0,width:30,shadow:true,double:true};
  const tokenPresets={
    classic:{ring:'#b79661',bg:'#171513',accent:'#e3c68f',width:30,shape:'circle',double:true},
    iron:{ring:'#666d73',bg:'#17191b',accent:'#c7ccd0',width:34,shape:'circle',double:true},
    crimson:{ring:'#7b181a',bg:'#1e0c0d',accent:'#df9b79',width:35,shape:'circle',double:true},
    arcane:{ring:'#51428f',bg:'#111127',accent:'#a99cf4',width:32,shape:'circle',double:true},
    nature:{ring:'#49613c',bg:'#11190e',accent:'#accb83',width:34,shape:'circle',double:true},
    shadow:{ring:'#27272c',bg:'#070709',accent:'#74727f',width:40,shape:'circle',double:true},
    minimal:{ring:'#d5d0c6',bg:'#161616',accent:'#d5d0c6',width:12,shape:'circle',double:false},
    borderless:{ring:'#000000',bg:'#151515',accent:'#000000',width:0,shape:'circle',double:false},
  };
  function tokenPath(ctx,shape,inset=36){
    const s=720, cx=s/2,cy=s/2,r=s/2-inset;ctx.beginPath();
    if(shape==='square'){const x=inset,y=inset,w=s-inset*2,rad=72;ctx.roundRect(x,y,w,w,rad)}
    else if(shape==='hex'){for(let i=0;i<6;i++){const a=-Math.PI/2+i*Math.PI/3,x=cx+Math.cos(a)*r,y=cy+Math.sin(a)*r;i?ctx.lineTo(x,y):ctx.moveTo(x,y)}ctx.closePath()}
    else ctx.arc(cx,cy,r,0,Math.PI*2);
  }
  function syncTokenControls(){
    const map=[['shape','[data-token-shape]'],['ring','[data-token-ring]'],['bg','[data-token-bg]'],['accent','[data-token-accent]'],['scale','[data-token-scale]'],['x','[data-token-x]'],['y','[data-token-y]'],['width','[data-token-width]']];
    map.forEach(([k,s])=>{const el=$(s);if(el)el.value=tokenSettings[k]});
    const shadow=$('[data-token-shadow]'),dbl=$('[data-token-double]');if(shadow)shadow.checked=tokenSettings.shadow;if(dbl)dbl.checked=tokenSettings.double;
    $('[data-token-scale-out]').textContent=`${Math.round(tokenSettings.scale*100)}%`;$('[data-token-x-out]').textContent=String(tokenSettings.x);$('[data-token-y-out]').textContent=String(tokenSettings.y);$('[data-token-width-out]').textContent=String(tokenSettings.width);
  }
  function drawToken(){
    ...
    const img=state.tokenImage;if(img){const base=Math.max(s/img.naturalWidth,s/img.naturalHeight)*Number(tokenSettings.scale||1);const w=img.naturalWidth*base,h=img.naturalHeight*base,x=(s-w)/2+Number(tokenSettings.x||0),y=(s-h)/2+Number(tokenSettings.y||0);ctx.drawImage(img,x,y,w,h)}ctx.restore();
    const width=Number(tokenSettings.width||0);if(width>0){ctx.save();if(tokenSettings.shadow){ctx.shadowColor='rgba(0,0,0,.7)';ctx.shadowBlur=22;ctx.shadowOffsetY=10}tokenPath(ctx,tokenSettings.shape,36+width/2);ctx.strokeStyle=tokenSettings.ring;ctx.lineWidth=width;ctx.stroke();ctx.restore();if(tokenSettings.double&&width>8){tokenPath(ctx,tokenSettings.shape,36+width+8);ctx.strokeStyle=tokenSettings.accent;ctx.lineWidth=Math.max(3,width*.14);ctx.stroke()}}
  ...
  const tokenControlMap={'[data-token-shape]':'shape','[data-token-ring]':'ring','[data-token-bg]':'bg','[data-token-accent]':'accent','[data-token-scale]':'scale','[data-token-x]':'x','[data-token-y]':'y','[data-token-width]':'width'};
