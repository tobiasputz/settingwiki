(()=>{
  ...
  function renderDetail(){
    ...
    detail.innerHTML=`<span class="eyebrow">${esc(n.chapter)}</span><h2>${esc(n.title)}</h2><p>${near.length} direct ${near.length===1?'connection':'connections'} in this view.</p><a class="network-open-entry" href="/wiki/${encodeURIComponent(n.slug)}">Open Codex entry →</a><div>${near.slice(0,24).map(x=>{const other=bySlug.get(x.slug);return `<button class="network-related" data-focus-slug="${escAttr(x.slug)}"><small>${esc(x.edge.explicit?(x.edge.label||x.edge.relation||'related'):'mentioned')}</small><strong>${esc(other?.title||x.slug)}</strong></button>`}).join('')||'<span class="network-muted">No direct links in the current filter.</span>'}</div>`;
    detail.querySelectorAll('[data-focus-slug]').forEach(b=>b.onclick=()=>selectNode(b.dataset.focusSlug));
  }
