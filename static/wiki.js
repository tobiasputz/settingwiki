(()=>{
  ...
  document.addEventListener('keydown',e=>{
    ...
    if(overlay?.classList.contains('open')&&['ArrowDown','ArrowUp','Enter'].includes(e.key)){
      ...
      if(e.key==='Enter'&&searchIndex>=0){const target=anchors[searchIndex];if(target.tagName==='BUTTON')target.click();else if(target.href)location.href=target.href;}
    }
  });
  ...
  const setMoreSheet=open=>{if(!moreSheet)return;const active=!!open;moreSheet.classList.toggle('open',active);moreSheet.setAttribute('aria-hidden',active?'false':'true');document.body.classList.toggle('mobile-more-open',active);document.documentElement.classList.toggle('mobile-more-open',active);if(active){const scroller=moreSheet.querySelector('[data-mobile-sheet-scroll]');if(scroller)scroller.scrollTop=0}};
  ...
  moreSheet?.addEventListener('touchmove',e=>{if(moreSheet.classList.contains('open'))e.stopPropagation()},{passive:true});
  ...
  // Campaign Workspace extends the palette with tracked objects and workspace destinations.
  let v8PaletteTimer;input?.addEventListener('input',()=>{clearTimeout(v8PaletteTimer);const q=input.value.trim();if(!adminMode||!q)return;v8PaletteTimer=setTimeout(async()=>{try{const rows=await fetch('/api/v8/command?q='+encodeURIComponent(q),{cache:'no-store'}).then(r=>r.ok?r.json():[]);const existing=new Set([...results.querySelectorAll('.search-result')].map(x=>(x.getAttribute('href')||'')+'|'+(x.querySelector('strong')?.textContent||'')));const fresh=(rows||[]).filter(x=>x.href&&!existing.has(x.href+'|'+x.title)).slice(0,12);if(!fresh.length)return;const html=fresh.map(x=>`<a class="search-result command-result v8-command-result" href="${escapeHtml(x.href)}"><small>${escapeHtml(x.subtitle||'Campaign Workspace')}<span class="search-type">${escapeHtml(x.type||'object')}</span></small><strong>${escapeHtml(x.title)}</strong><p>${escapeHtml(x.excerpt||'Tracked object or workspace action')}</p></a>`).join('');results.insertAdjacentHTML('afterbegin',html)}catch(_){ }},210)});
  // V6 command actions stay server-authorized. Typing “advance clock X” or
  // “reveal Y” offers an executable GM action without exposing write logic to players.
  let v6CommandTimer;input?.addEventListener('input',()=>{clearTimeout(v6CommandTimer);const q=input.value.trim();if(!adminMode||!(q.toLowerCase().startsWith('advance clock ')||q.toLowerCase().startsWith('reveal ')))return;v6CommandTimer=setTimeout(async()=>{try{const rows=await fetch('/api/v6/commands?q='+encodeURIComponent(q),{cache:'no-store'}).then(r=>r.json());const actions=rows.filter(x=>x.type==='action'&&x.action);if(!actions.length)return;const html=actions.map((x,i)=>`<button type="button" class="search-result command-result command-action" data-v6-command='${escapeHtml(JSON.stringify(x.action))}'><small>${escapeHtml(x.chapter||'GM action')}<span class="search-type">action</span></small><strong>${escapeHtml(x.title)}</strong><p>${escapeHtml(x.excerpt||'')}</p></button>`).join('');results.insertAdjacentHTML('afterbegin',html);results.querySelectorAll('[data-v6-command]').forEach(b=>b.onclick=async()=>{let action={};try{action=JSON.parse(b.dataset.v6Command||'{}')}catch{};if(!confirm(b.querySelector('strong')?.textContent+'?'))return;b.disabled=true;try{const r=await fetch('/api/v6/commands/action',{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify(action)});if(!r.ok)throw Error((await r.json().catch(()=>({}))).detail||'Action failed');b.querySelector('p').textContent='Done.';setTimeout(close,500)}catch(err){b.disabled=false;b.querySelector('p').textContent=err.message}}) }catch(_){ }},230)});
  ...
  pollNotifications();setInterval(pollNotifications,60000);document.addEventListener('visibilitychange',()=>{if(!document.hidden)pollNotifications()});document.addEventListener('seeker:event',e=>{const p=e.detail?.path||'';if(/notification|handout|reveal|session|spotlight/.test(p))pollNotifications()});
