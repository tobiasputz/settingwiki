(()=>{
  ...
  const searchLanding=()=>{
    ...
    results.innerHTML=`<div class="palette-home"><div class="palette-home-section"><small class="palette-home-label">Quick jumps</small><div class="palette-quick-grid"><a href="/session">✦ Session</a><a href="/campaign">◇ Chronicle</a><a href="/characters">♟ Characters</a><a href="/mysteries">? Mysteries</a></div></div>${recent.length?`<div class="palette-home-section"><small class="palette-home-label">Recently viewed</small><div class="palette-recent-list">${recent.map(x=>`<a href="${escapeHtml(x.href||('/wiki/'+x.slug))}"><strong>${escapeHtml(x.title||'Lore')}</strong><small>${escapeHtml(x.chapter||'Setting')}</small></a>`).join('')}</div></div>`:''}</div>`;
  };
  ...
  const notificationHref=n=>n.target_type==='page'||n.target_type==='lore'?('/wiki/'+n.target_key):n.target_type==='character'?('/characters/'+n.target_key):n.target_type==='thread'?('/campaign#threads'):n.target_type==='map'?('/atlas/'+n.target_key):'';
  const renderNotifications=()=>{if(!notificationFeed)return;const unread=notificationRows.filter(x=>!x.read);if(notificationCount){notificationCount.textContent=unread.length?String(Math.min(99,unread.length)):'';notificationCount.classList.toggle('show',!!unread.length)}notificationFeed.innerHTML=notificationRows.length?notificationRows.map(n=>`<article class="notification-item kind-${escapeHtml(n.kind)} ${n.read?'read':''}" data-notification-row="${n.id}"><small>${escapeHtml((n.kind||'notice').toUpperCase())}</small><strong>${escapeHtml(n.title)}</strong><p>${escapeHtml(n.body||'')}</p><div>${notificationHref(n)?`<a href="${escapeHtml(notificationHref(n))}">Open →</a>`:''}<button data-notification-read="${n.id}">${n.read?'Read':'Mark read'}</button></div></article>`).join(''):'<div class="search-hint">No campaign notifications yet.</div>';notificationFeed.querySelectorAll('[data-notification-read]').forEach(b=>b.onclick=async()=>{await fetch('/api/public/notifications/'+b.dataset.notificationRead+'/read',{method:'POST',headers:{'Content-Type':'application/json'},body:'{}'}).catch(()=>{});const n=notificationRows.find(x=>String(x.id)===b.dataset.notificationRead);if(n)n.read=true;renderNotifications()})};
