...
def test_v5_frontend_release_contracts():
    ...
    assert 'INVESTIGATION' in investigation and 'investigation.js?v=6000' in investigation
    assert 'Previously on' in recap
    assert 'Pathbuilder / sheet URL' in chars and 'Foundry actor URL' in chars and 'shAttacks' not in (root/'static/characters.js').read_text()
    ...
    assert 'seeker-static-v6000' in sw and "'/investigation','/recap'" in sw
