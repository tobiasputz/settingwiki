...
def test_v7_release_identity_assets_and_bridge():
    ...
    assert (root/'VERSION').read_text().strip()=='7.2.0'
    assert 'seeker-static-v7200' in (root/'static/sw.js').read_text(encoding='utf-8')
...
def test_v7_http_gm_and_player_privacy(tmp_path: Path,monkeypatch):
    ...
    assert gm.get('/api/v7/health').json()['version']=='7.2.0'
