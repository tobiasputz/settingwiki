...
def test_v8_release_contract_and_assets():
    ...
    assert (root/'VERSION').read_text().strip()=='10.2.0'
    sw=(root/'static/sw.js').read_text(encoding='utf-8')
    assert 'seeker-static-v10200' in sw and '/static/v8.js?v=10200' in sw
...
def test_v8_http_workspace_source_studio_portal_and_command_palette(tmp_path: Path,monkeypatch):
    ...
    state=gm.get('/api/v8/state');assert state.status_code==200 and state.json()['version']=='10.2.0'
